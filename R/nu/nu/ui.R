...
page <- dashboardPage(skin = 'blue', 
                    ...
                    # Header --------------------------------------------------------------------------------------------------------------------
                    dashboardHeader(title='Mamíferos: areas de hábitat', titleWidth = 300,
                                    dropdownMenuOutput("avanceEspecies")),
                    # Menúes laterales -------------------------------------------------------------------------------------------------------------
                    ...
                    # Contenidos páginas principales -----------------------------------------------------------------------------------------------
                    dashboardBody(
                      tags$head(
                        tags$style(
                          HTML(
                                      ...
                                      fluidRow(
                                        ...
                                        column(3, div(style = "height:50px;",
                                                      fileInput('shapesElev', label = NULL, multiple = F, accept = '.kml',
                                                                buttonLabel='Explorar', width= '100%')))),
                                      leafletOutput("mapadatosE", height = 700) %>% withSpinner()
                                  )
                                )
                        ),
                        # tabResumen ---------------------------------------------------------------------------------------------------------------
                                  ...
                                  column(3,
                                         box(width=NULL, title = "Finalizar", status = 'success', solidHeader = F, useShinyalert(),
                                             ...
                                             column(8, offset = 2,
                                                    ...
                                                    actionBttn("stop", "Finalizar y generar informe",
                                                               style="jelly", color = 'primary', size = 'md', block=T)))
                                  )
                                )
                                ...
                                tabBox(title = 'Calculador y visualizador de AOH', width=12, height = 800,
                                       tabPanel(title = 'AOH', 
                                                fluidRow(column(2, uiOutput('familiaAOH', inline = T)),
                                                         ...
                                                         # column(4,includeScript("js/scroll.js"), includeCSS("css/styles.css"),
                                                         #        div(id = "wallaceLog", class = "scrollbox", 
                                                         #            style = "height:100px;background-color: gray70;", 
                                                         #            htmlOutput("log")))
                                                ),
                                                fluidRow(br(),
